...
def checksave(savefile: str):
    ...
    try:
        ...
        if savelist[0].split("=")[0] == "name" and len(savelist[0].split("=")[1]) > 2 and savelist != [] and savelist[1].split("=")[0]== "avatar" and len(savelist[1].split("=")[1]) > 2 and tilenames.__contains__(savelist[1].split("=")[1][:-1]):
            savedata["valid"] = True
...
def main():
    ...
    while introrunning:
        ...
        if ((savedata[0]["valid"] or savedata[1]["valid"] or savedata[2]["valid"]) and 0 <= savechosen <= 2 and ticks > 100) or (not introrunning):
            introrunning = False
